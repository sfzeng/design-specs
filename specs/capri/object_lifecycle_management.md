...
| Transition | This action specifies a period in the objects' lifetime when OpenSDS should transition them to the STANDARD_IA, ONEZONE_IA, INTELLIGENT_TIERING, or the GLACIER storage class. Type: Container Children: Days, StorageClass, and Backend Ancestor: Rule | Yes, if no other action is present in the Rule. |
| Expiration | This action specifies a period in object's lifetime when OpenSDS should take the appropriate expiration action. Type: Container Children: Days Ancestor: Rule | Yes, if no other action is present in the Rule. |
